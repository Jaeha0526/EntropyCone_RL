...
# Collect multirun data
multirun_results = {}  # {samples: [(r2, cos), ...]}

for samples in [10, 20, 30, 40, 50, 60, 70]:
    multirun_results[samples] = []
    for run in range(1, 6):
        path = os.path.join(base_dir, f'samples{samples}_multirun/run{run}/gradient_samples_stage_0.json')
...
if multi_samples:
    ax1.errorbar(multi_samples, multi_r2_mean, yerr=multi_r2_std,
                 ...
                 capsize=5, capthick=2, label='Multirun (5 runs, mean±std)')
...
plt.suptitle('Gradient Quality vs Sample Size (with error bars)\ndS=0.01, max1, n=3 symmetric point', fontsize=14, y=1.02)
plt.tight_layout()

# Save plot
output_path = os.path.join(script_dir, 'samples_vs_gradient_quality_multirun.png')
plt.savefig(output_path, dpi=150, bbox_inches='tight')
print(f"Saved: {output_path}")

output_pdf = os.path.join(script_dir, 'samples_vs_gradient_quality_multirun.pdf')
...
# Print summary table
print("\n" + "="*70)
print("MULTIRUN RESULTS (5 runs each):")
print("-"*70)
