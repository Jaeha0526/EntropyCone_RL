#!/usr/bin/env python3
"""
Plot R² and Cosine similarity vs dS for max10 experiments.
"""
...
# Collect data for max10 experiments
results = []
script_dir = os.path.dirname(os.path.abspath(__file__))
base_dir = os.path.dirname(script_dir)

for path in sorted(glob.glob(os.path.join(base_dir, '*_max10/gradient_samples_stage_0.json'))):
...
# Title and grid
plt.title('Gradient Quality vs Perturbation Size (dS)\nmax10 experiments (10x max pooling), n=3 symmetric point', fontsize=14)
...
for i, (dS, r2, cos) in enumerate(results):
    if dS in [0.003, 0.01]:
        ax1.annotate(f'dS={dS}\nR²={r2:.3f}',
                    xy=(dS, r2), xytext=(dS*1.5, r2-0.02),
...
# Save plot
output_path = os.path.join(script_dir, 'dS_vs_gradient_quality_max10.png')
plt.savefig(output_path, dpi=150, bbox_inches='tight')
print(f"Saved: {output_path}")

# Also save as PDF
output_pdf = os.path.join(script_dir, 'dS_vs_gradient_quality_max10.pdf')
